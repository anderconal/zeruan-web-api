# -*- coding: utf-8 -*-
# Generated by Django 1.11.2 on 2017-07-26 06:57
from __future__ import unicode_literals

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        ('invoice_details', '0002_auto_20170725_2129'),
    ]

    operations = [
        migrations.AlterField(
            model_name='invoicedetail',
            name='prepaid_card',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, related_name='prepaid_card', to='products.PrepaidCard'),
        ),
        migrations.AlterField(
            model_name='invoicedetail',
            name='product',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, related_name='product', to='products.Product'),
        ),
        migrations.AlterField(
            model_name='invoicedetail',
            name='service',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='services.Service'),
        ),
    ]
